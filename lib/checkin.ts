...
import { runPostCheckinSideEffects } from "@/lib/checkin-side-effects";
import {
...
async function performCheckinOnRegistration(
  ...
  const txResult = await prisma.$transaction(async (tx) => {
    ...
    try {
      ...
      if (
        ...
        const raceAfterConflict = await tx.checkin.findFirst({
          where: { registrationId: reg.id },
          orderBy: { createdAt: "desc" },
        });
        if (raceAfterConflict) {
          return {
            kind: "already_checked_in" as const,
            checkin: raceAfterConflict,
          };
        }
      }
      throw err;
  ...
  void runPostCheckinSideEffects(
    {
      registrationId: reg.id,
...
export async function processCheckinScan(
  ...
  const normalized = normalizeCheckinToken(token);
  ...
  const workshopValidation = validateExpectedWorkshopDate({
    passWorkshopDateId: pass.registration.workshopDateId,
    expectedWorkshopDateId: meta.expectedWorkshopDateId,
  });
  if (!workshopValidation.ok) {
    return {
      ok: false,
      error: workshopValidation.error,
      code: workshopValidation.code,
    };
  }
