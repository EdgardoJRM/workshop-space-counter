...
/** Convierte texto plano (párrafos separados por línea en blanco) a HTML simple para SES. */
export function plainTextToEmailHtml(text: string): string {
  const normalized = text.replace(/\r\n/g, "\n").trim();
  if (!normalized) return "";

  if (HAS_HTML_TAG.test(normalized)) {
    return normalized;
  }
...
/** Muestra plantillas guardadas como HTML en un campo de texto normal. */
